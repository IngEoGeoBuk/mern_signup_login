...
const Best = () => {
        ...
        .then((res) => res.data.forEach((poId: any) => {
            ...
            .then((res2 : any) => {
                setPostList((postList : any ) => [
                    ...postList,
                    { _id: res2.data[0]._id, title: res2.data[0].title, content: res2.data[0].content, time: res2.data[0].time }
                ])
            })
        }))
    }, [])
    const newPostList = postList.sort((a, b) => (a.time > b.time) ? 1 : -1)
    ...
    const displayPosts = newPostList.reverse()
